using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using Godot;

public class MeshGenerator
{
    private static MeshGenerator _instance;
    public static MeshGenerator Instance => _instance ?? new MeshGenerator();

    public ArrayMesh GenerateMesh(float[,] heightMap, float scale, Curve heightCurve, int lod)
    {
        var vertexStepIncrement = lod == 0 ? 1 : lod * 2;
        var width = heightMap.GetLength(0);
        var mapSize = (width - 1) / vertexStepIncrement + 1;

        var topLeftX = -(width - 1) * .5f;
        var topLeftY = -(width - 1) * .5f;

        var surfaceTool = new SurfaceTool();
        surfaceTool.Begin(Mesh.PrimitiveType.Triangles);

        var vertexIndex = 0;
        for (int y = 0; y < width; y += vertexStepIncrement)
        {
            for (int x = 0; x < width; x += vertexStepIncrement)
            {
                surfaceTool.SetSmoothGroup(uint.MaxValue);
                surfaceTool.SetUV(new(x / (float)width, y / (float)width));
                surfaceTool.AddVertex(new(topLeftX + x, heightCurve.Sample(heightMap[x, y]) * scale, topLeftY + y));

                if (x < width - 1 && y < width - 1)
                {
                    surfaceTool.AddIndex(vertexIndex);
                    surfaceTool.AddIndex(vertexIndex + 1);
                    surfaceTool.AddIndex(vertexIndex + mapSize + 1);
                    surfaceTool.AddIndex(vertexIndex + mapSize + 1);
                    surfaceTool.AddIndex(vertexIndex + mapSize);
                    surfaceTool.AddIndex(vertexIndex);
                }
                vertexIndex++;
            }
        }

        surfaceTool.GenerateNormals();
        surfaceTool.GenerateTangents();

        return surfaceTool.Commit();
    }
}